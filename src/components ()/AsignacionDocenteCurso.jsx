...
const AsignacionDocenteCurso = () => {
  ...
  const agregarCurso = async () => {
    ...
    const { data, error } = await supabase.from("cursos").insert({ nombre: nuevoCurso }).select();
    if (!error && data) {
  ...
  const editarHoras = async (cursoId, gradoId, horas) => {
    await supabase
      ...
      .upsert({ curso_id: cursoId, grado_id: gradoId, horas: parseInt(horas) });
    cargarHorasCursoGrado();
  };
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto">
      ...
      {/* Tabla de horas por curso y grado */}
